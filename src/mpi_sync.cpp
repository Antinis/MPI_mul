...
int main()
{
    ...
    // if(N%4!=0)
    // {
    //     master_len=N/2+1;       // 任意数
    //     slave_len=N-master_len; // 4的倍数
    // }
    // else
    // {
    //     master_len=N/2;       // 任意数
    ...
    if(rank==0) // master管理节点，同时作为slave 1计算节点
    {
        ...
        for (int k=0;k<n;++k)
        {
            ...
            {
                // 左矩阵发送
                MPI_Isend(&matC[(slave)/2*N*slave_len], N*slave_len, MPI_INT, slave, 0, MPI_COMM_WORLD, &request);
                // 右矩阵发送
                MPI_Isend(&matA[(slave)%2*N*slave_len], N*slave_len, MPI_INT, slave, 1, MPI_COMM_WORLD, &request);
            }
            send_end=clock();
            ...
            for(int slave=1; slave<=world_size-1; slave++)
            {
                MPI_Recv(RecvTmp, slave_len*slave_len, MPI_INT, slave, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                ...
                for(int i=0; i<slave_len; i++)
                    for(int j=0; j<slave_len; j++)
                        matC2[(i+bias_i)*N+j+bias_j]=RecvTmp[i*slave_len+j];
            }
            recv_end=clock();
